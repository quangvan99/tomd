        ...
        with self._lock:
            if key not in self._models:
                ...
                if backend == "transformers":
                    ...
                    from qmd_vl_utils.mlx_compat import load_mlx_model
                    ...
                    if backend == "vllm-engine":
                        ...
                        if enable_custom_logits_processors() and ("logits_processors" not in kwargs):
                            from qmd_vl_utils import MinerULogitsProcessor
                ...
                predictor._qmd_runtime_handles = {
                    "backend": backend,
                    "model": model,
                    "processor": processor,
                    "vllm_llm": vllm_llm,
...
def _iter_shutdown_candidates(predictor: MinerUClient):
    runtime_handles = getattr(predictor, "_qmd_runtime_handles", {})
...
def _maybe_enable_serial_execution(
    ...
    if _predictor_uses_mlx(predictor, backend) and not hasattr(
        predictor, "_qmd_execution_lock"
    ):
        predictor._qmd_execution_lock = threading.Lock()
    return predictor


@contextmanager
def predictor_execution_guard(predictor: MinerUClient):
    lock = getattr(predictor, "_qmd_execution_lock", None)
    if lock is None:
        yield
        return
    with lock:
