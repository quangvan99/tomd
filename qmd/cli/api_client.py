...
class LocalAPIServer:
    def __init__(self, extra_cli_args: Sequence[str] = ()):
        self.temp_dir = tempfile.TemporaryDirectory(prefix="qmd-api-client-")
...
async def wait_for_local_api_ready(
    ...
    while asyncio.get_running_loop().time() < deadline:
        ...
        if process is not None and process.poll() is not None:
            raise click.ClickException(
                "Local qmd-api exited before becoming healthy."
            )
        try:
    ...
    message = "Timed out waiting for local qmd-api to become healthy."
    if last_error:
...
async def download_result_zip(
    ...
    zip_fd, zip_path = tempfile.mkstemp(suffix=".zip", prefix="qmd_cli_result_")
    os.close(zip_fd)
