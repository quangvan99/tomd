...
from qmd.cli.api_client import (
...
from qmd.cli.api_protocol import API_PROTOCOL_VERSION
from qmd.cli.common import normalize_upload_filename
...
class WorkerPool:
    ...
    async def start(self) -> None:
        ...
        if self._monitor_task is None or self._monitor_task.done():
            self._monitor_task = asyncio.create_task(self._monitor_loop(), name="qmd-router-worker-monitor")
...
class RouterTaskRegistry:
    ...
    async def start(self) -> None:
        ...
        if self._cleanup_task is None or self._cleanup_task.done():
            self._cleanup_task = asyncio.create_task(self._cleanup_loop(), name="qmd-router-task-cleanup")
...
def warn_if_router_preload_ignored(settings: RouterSettings) -> None:
    if settings.enable_vlm_preload and not parse_local_gpus(settings.local_gpus):
        logger.warning(
            "Ignoring --enable-vlm-preload because qmd-router is not launching any local qmd-api workers."
        )
...
async def stage_multipart_request(request: Request) -> MultipartPayload:
    temp_dir = tempfile.mkdtemp(prefix="qmd-router-request-")
    uploads: list[StagedUpload] = []
    fields: list[tuple[str, str]] = []
...
@click.option(
    "--worker-host",
    default="127.0.0.1",
    help="Host for router-managed qmd-api workers (default: 127.0.0.1).",
)
@click.option(
    "--enable-vlm-preload",
    "enable_vlm_preload",
    type=bool,
    default=False,
    help="Preload the local VLM model in router-managed qmd-api workers.",
)
def main(
    ctx: click.Context,
    host: str,
