...
from qmd.cli import api_client as _api_client
from qmd.cli.output_paths import resolve_parse_dir
from qmd.cli.vlm_preload import resolve_gradio_local_api_cli_args
from qmd.cli.visualization import VisualizationJob, run_visualization_job
...
async def resolve_server_health(http_client, api_url):
    ...
    if started_now:
        logger.info(f"Started local qmd-api at {local_server.base_url}")
...
@click.option(
    ...
    'api_url',
    type=str,
    help="MinerU FastAPI base URL. If omitted, gradio starts a reusable local qmd-api service.",
    default=None,
)
@click.option(
    '--enable-vlm-preload',
    'enable_vlm_preload',
    type=bool,
    help="Preload the local VLM model when gradio starts a local qmd-api service.",
    default=False,
)
@click.option(
