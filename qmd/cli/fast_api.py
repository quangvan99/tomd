...
from qmd.cli.public_http_client_policy import (
...
from qmd.cli.output_paths import resolve_parse_dir
from qmd.cli.api_protocol import (
    API_PROTOCOL_VERSION,
    DEFAULT_MAX_CONCURRENT_REQUESTS,
    DEFAULT_PROCESSING_WINDOW_SIZE,
)
from qmd.cli.vlm_preload import (
    maybe_preload_vlm_model,
    split_service_and_model_config,
)
from qmd.backend.vlm.vlm_analyze import shutdown_cached_models
from qmd.utils.cli_parser import arg_parse
from qmd.utils.check_sys_env import is_mac_environment
from qmd.utils.config_reader import (
...
def create_result_zip(
    ...
    zip_fd, zip_path = tempfile.mkstemp(suffix=".zip", prefix="qmd_results_")
    os.close(zip_fd)
    ...
    async def start(self) -> None:
        ...
        if self.dispatcher_task is None or self.dispatcher_task.done():
            self.dispatcher_task = asyncio.create_task(
                self._dispatcher_loop(), name="qmd-fastapi-task-dispatcher"
            )
        if (
            ...
            self.cleanup_task = asyncio.create_task(
                self._cleanup_loop(), name="qmd-fastapi-task-cleanup"
            )
        ...
        try:
            while True:
                ...
                processor = asyncio.create_task(
                    ...
                    name=f"qmd-fastapi-task-{task_id}",
                )
...
@click.option(
    ...
    type=bool,
    default=False,
    help="Preload the local VLM model during qmd-api startup.",
)
def main(
    ctx,
    host,
