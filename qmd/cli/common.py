...
from qmd.utils.draw_bbox import draw_layout_bbox, draw_span_bbox
from qmd.utils.engine_utils import get_vlm_engine
from qmd.utils.enum_class import MakeMode
from qmd.utils.guess_suffix_or_lang import guess_suffix_by_bytes
from qmd.utils.pdf_image_tools import images_bytes_to_pdf_bytes
from qmd.backend.vlm.vlm_middle_json_mkcontent import union_make as vlm_union_make
from qmd.backend.office.office_middle_json_mkcontent import union_make as office_union_make
from qmd.backend.vlm.vlm_analyze import doc_analyze as vlm_doc_analyze
from qmd.backend.vlm.vlm_analyze import aio_doc_analyze as aio_vlm_doc_analyze
from qmd.backend.office.pptx_analyze import office_pptx_analyze
from qmd.backend.office.xlsx_analyze import office_xlsx_analyze
from qmd.backend.office.docx_analyze import office_docx_analyze
from qmd.utils.pdfium_guard import rewrite_pdf_bytes_with_pdfium
...
def build_hybrid_dependency_error_message(backend: str) -> str:
    return (
        f"`{backend}` requires local pipeline dependencies (`qmd[pipeline]`, "
        "including `torch`). Install `qmd[pipeline]` or `qmd[core]`. "
...
def _load_hybrid_analyze_entrypoint(entrypoint_name: str, backend: str):
    ...
    try:
        hybrid_analyze = importlib.import_module("qmd.backend.hybrid.hybrid_analyze")
...
def _process_output(
    ...
    from qmd.backend.pipeline.pipeline_middle_json_mkcontent import union_make as pipeline_union_make
...
def _process_pipeline(
    ...
    """处理pipeline后端逻辑"""
    from qmd.backend.pipeline.pipeline_analyze import doc_analyze_streaming as pipeline_doc_analyze_streaming
