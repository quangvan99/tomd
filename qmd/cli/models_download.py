...
def configure_model(model_dir, model_type):
    """配置模型"""
    json_url = 'https://gcore.jsdelivr.net/gh/opendatalab/MinerU@master/qmd.template.json'
    config_file_name = os.getenv('MINERU_TOOLS_CONFIG_JSON', 'qmd.json')
...
def get_effective_download_model_source(requested_model_source):
    ...
    if current_model_source == 'local':
        logger.warning(
            ...
            f"`qmd-models-download` will temporarily use '{requested_model_source}' "
